...
parser.add_argument('--method', type=str,
                    ...
                    choices=[ 'rollout', 'lrp','transformer_attribution', 'attCat_attribution', 'generate_attribution', 'integrated_attribution', 'full_lrp', 'lrp_last_layer',
...
def eval_batch(image, labels, evaluator, index):
    ...
    elif args.method == 'attCat_attribution':
        Res = lrp.generate_TransCAM(image.cuda(), start_layer=1).reshape(batch_size, 1, 14, 14)

    elif args.method == 'generate_attribution':
        Res = lrp.generate_attribution(image.cuda(), start_layer=4).reshape(batch_size, 1, 14, 14)

    elif args.method == 'integrated_attribution':
        Res = lrp.integrated_attribution(image.cuda(), start_layer=4).reshape(batch_size, 1, 14, 14) 
